...
void InvAddRoundKey(uint8_t round, state_t* state, const uint8_t* RoundKey){
    uint8_t i, j;
    for (i = 0; i < 4; ++i)    {
        for (j = 0; j < 4; ++j)        {
...
void Cipher_biterror(state_t* state, const uint8_t RoundKey[AES_keyExpSize], uint8_t byte, uint8_t err){
    ...
    int i = byte % 4, j = byte / 4;
    (*state)[i][j] = (*state)[i][j] ^ err;
    //std::cout << "\nM9: \t";  phex(*state);
...
void Cipher_byteerror(state_t* state, const uint8_t RoundKey[AES_keyExpSize], uint8_t nbyte, uint8_t err) {
    uint8_t round = 0;

    // Add the First round key to the state before starting the rounds.
    AddRoundKey(0, state, RoundKey);

    // Fault is injected in round Nr-1 :: on the byte nbyte of state M8
    int i = nbyte % 4, j = nbyte / 4;
...
void Cipher_1R(state_t* state,  const uint8_t RoundKey[AES_keyExpSize])
{
    AddRoundKey(0, state, RoundKey);
    #ifdef _PRINT_CIPHER 
        cout << "\nAddK0  : "; phex((uint8_t*) state, 16);
    #endif
    SubBytes(state);
    #ifdef _PRINT_CIPHER 
        cout << "\nSubByts: "; phex((uint8_t*) state, 16);
    #endif
    ShiftRows(state);
    #ifdef _PRINT_CIPHER
        cout << "\nShiftR : "; phex((uint8_t*) state, 16);
    #endif
    MixColumns(state);
    #ifdef _PRINT_CIPHER 
        cout << "\nMixCol : "; phex((uint8_t*) state, 16);
    #endif
    AddRoundKey(1, state, RoundKey);
    #ifdef _PRINT_CIPHER
        cout << "\nAddK1  : "; phex((uint8_t*) state, 16);
    #endif
